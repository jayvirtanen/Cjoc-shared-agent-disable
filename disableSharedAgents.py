...
if not(jenkins_url.startswith("http://") or jenkins_url.startswith("https://")):
    ...
    sys.exit()
try:
    response = requests.get(jenkins_url+'/cjoc/view/all/api/json',auth=HTTPBasicAuth(username, apiToken), verify=False)
    print(response.status_code)
    if (response.status_code == 200 and answers["Action Type"] == "Disable"):
        i=0
        crumb = requests.get(jenkins_url+'/cjoc/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,":",//crumb)',auth=HTTPBasicAuth(username, apiToken), verify=False)
